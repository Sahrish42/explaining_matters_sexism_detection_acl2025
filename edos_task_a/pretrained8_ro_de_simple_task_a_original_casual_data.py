...
m_name = 'pretrained8_original_causal_eval_ro_de_simple'
FOLDER_NAME = 'casual_analysis'
...
if CASUAL_AND_ORIGINAL:
    print('----------------------- Original, Casual and Eval Data------------------------')
    ...
    analysis_data = read_casual_data(
        os.path.join(PATH, FOLDER_NAME, 'data'), col='combine')
    print(f"causal analysis data: {analysis_data.shape}")

    train_data = pd.concat([train_data, analysis_data, eval_data], ignore_index=True)
...
eval_data['label_ids'] = [label_vector_map[i[1]] for i in eval_data['label_sexist'].items()]
test_data['label_ids'] = [label_vector_map[i[1]] for i in test_data['label_sexist'].items()]
num_labels = len(train_data['label_sexist'].unique())
...
DEBERTA_PRETRAINED_MODEL_NAME = os.path.join(PATH, 'deberta_pretraining_2m','deberta-v3-pretrained', 'deberta_checkpoint-epoch-53')

ROBERTA_MODEL_NAME = 'roberta-large'
ROBERTA_PRETRAINED_MODEL_NAME = os.path.join(PATH, 'deberta_pretraining_2m','roberta-pretrained', 'roberta_checkpoint-epoch_8')
...
r_model = AutoModel.from_pretrained(ROBERTA_PRETRAINED_MODEL_NAME,ignore_mismatched_sizes=True, output_hidden_states=True)
...
class SexistModel(nn.Module):
    ...
    def __init__(self, d_model, r_model):
        ...
        self.combine_head = nn.Linear(d_hidden_size + r_hidden_size, num_labels)
...
eval_dataset = SexistDataset(eval_dataframe, d_tokenizer, r_tokenizer, max_length)
...
checkpoint_dir = os.path.join(PATH, 'casual_analysis', 'models', m_name)
...
copy_and_paste = f"f1_macro_test: {f1_macro_test}, {m_name} max_length: {max_length}\n"
with open(os.path.join(PATH, FOLDER_NAME,'info.txt'), 'a') as f:
    f.write(copy_and_paste)

# model path
#edos_task10/casual_analysis/models/pretrained8_original_causal_eval_ro_de_simple
